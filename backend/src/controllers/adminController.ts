    ...
    const bookings = await Hotel.aggregate([
      ...
      { $match: { "bookings.status": { $nin: ["Pending", "Cancelled"] } } },
      {
        $group: {
          _id: {
            date: {
              $dateToString: { format: dateFormat, date: "$bookings.checkIn" },
            },
          },
...
export const getTopBookingHotels = async (req: Request, res: Response) => {
  try {
    ...
    const topHotels = await Hotel.aggregate([
      ...
      {
        $match: {
          "bookings.status": { $nin: ["Pending", "Cancelled"] },
          "bookings.checkIn": { $gte: dateRange },
        },
      },
      {
        $group: {
...
export const addCoupon = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      newCoupon.maxDiscount == null
    ) {
      return res.status(400).send({
        message: "Maximum Discount is required for percentage discount type",
      });
    }
...
export const updateCouponById = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res.status(400).send({
        message: "Max discount is required for percentage discount type",
      });
    }
...
export const getHotels = async (req: Request, res: Response) => {
  try {
    ...
    const hotels = await Hotel.find().skip(skip).limit(limit).exec();
